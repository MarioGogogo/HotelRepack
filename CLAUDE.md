...
这是一个使用 React Native 0.83.1 + TypeScript 构建的酒店管理系统移动应用，采用 Re.Pack 实现代码分割（code splitting）。
...
- **导航**: `src/navigation/RootNavigator.tsx` - 根导航器，包含登录页、首页和分包页面
- **屏幕目录**: `src/screens/` - 页面组件
  - `LoginScreen.tsx` - 登录页
  ...
  - `HotelDining/` - 餐饮服务模块
- **组件目录**: `src/components/` - 共享组件（ChunkErrorBoundary, UpdateDialog, Toast, Dialog, BackButton）
...
- `hotel-reservation` -> HotelReservation（客房预订）
- `hotel-cleaning` -> HotelCleaning（客房打扫）
- `hotel-dining` -> HotelDining（餐饮服务）
