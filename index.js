...
// 获取远程分包配置（供外部直接访问版本号）
export function getRemoteBundleConfig() {
  return remoteBundleConfig;
}

// 获取已加载的版本信息（供外部直接访问）
export function getLoadedVersions() {
...
// 从 AsyncStorage 恢复已持久化的版本号
AsyncStorage.getItem('loadedBundleVersions').then(stored => {
  if (stored) {
    try {
      const parsed = JSON.parse(stored);
      Object.assign(loadedVersions, parsed);
      console.log('[ScriptManager] Restored persisted versions:', loadedVersions);
    } catch (e) {
      console.warn('[ScriptManager] Failed to parse persisted versions:', e);
    }
  }
}).catch(console.error);

// 持久化版本号到 AsyncStorage
function persistLoadedVersions() {
  AsyncStorage.setItem('loadedBundleVersions', JSON.stringify(loadedVersions)).catch(console.error);
}
